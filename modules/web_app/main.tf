...
resource "azurerm_application_insights" "main" {
  ...
  name                = "${var.web_app_names[count.index]}-appinsights"
...
resource "azurerm_windows_web_app" "main" {
  ...
  site_config {
    application_stack {
      ...
      dotnet_version = var.dot_net_versions[count.index]
    }
...
resource "azurerm_windows_web_app_slot" "main" {
  count          = var.create_slots ? length(var.web_app_names) * length(local.slots) : 0
